...
**What tokenless reliably detects.** Count on the built-in structured detectors — emails, phone numbers, IBANs, card numbers, government IDs with checksums, secrets, crypto addresses. **Do not count on names, street addresses or other free-text PII in this mode:** those come from backend ML, and on the visitor route that contribution depends on how the backend is deployed and cannot be checked from the node — a response that carries no entities looks identical whether the tier is lite by design or its upstreams are down. If you need names and addresses masked, use API Key mode with Detection Mode `Auto` or `Cloud`. Note also that `Auto`/`Cloud` in tokenless still sends your text to the backend for scoring, so choosing them costs the privacy of `Local (Regex)` without a guarantee of anything in return.
...
- **Where:** `POST {baseUrl}/v1/telemetry/events` — the **same Tokenless Base URL the credential carries**. That URL defaults to `https://api.privent.ai`, so an unchanged credential sends here to **Privent Cloud**; self-hosters must set Base URL to their own deployment, and once they do, nothing reaches Privent.
