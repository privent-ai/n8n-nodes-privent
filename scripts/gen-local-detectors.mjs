...
// One vendored rule is REPLACED rather than copied. Upstream tests the
// placeholder word list against the whole value, so any address whose LOCAL PART
// contains test/demo/sample/example/… is dropped — `test.user@bank.invalid`,
// `sample.reports@clinic.invalid`, `demo.account@insurer.invalid` were all
// undetected, 3 of 3, measured through the node's own local path. Those are
// ordinary corporate mailboxes, not fixtures. What makes an address synthetic is
// its DOMAIN (RFC 2606 reserves example.com/.test/.invalid), so for EMAIL the
// word list is applied to the domain only; NAME keeps the whole-value check.
const PLACEHOLDER_RULE = /placeholder\|dummy\|mock/;
const PLACEHOLDER_MATCHER = `(value, context) => {
