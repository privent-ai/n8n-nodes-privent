...
| NP-X | **361 of 468 local detectors fire on nothing at all.** Measured while building N4-7b's admission rule: they produce zero false positives across 2,714 lines of negative corpus *and* zero hits across the positive corpus, so a zero-FP-only rule would have admitted them for looking harmless. They are shipped in the bundle and never match anything the corpora contain. The number is a floor, not a verdict: the positive corpus is 27 hand-authored cases, so "inert" here means "never fired on the cases we have", not "cannot fire". **The decision is framed here, not taken.** What it costs today, measured: the 361 inert detectors are **65.8 KB of the 118.5 KB generated file**, inside a **214.1 KB** published bundle — a third of what every installation downloads and parses is patterns that have never matched anything we have shown them. **Exit A, grow the positive corpus:** author positive cases per kind until each detector either proves itself or is shown inert against real evidence; 27 cases exist, 468 candidates remain, so this is hand-authoring at a scale nobody should do in one sitting, and it is the only exit that can ever ADMIT any of them. **Exit B, drop the inert set from the package:** one generator filter, immediate size win, and it makes exit A impossible later without reverting — a detector that is not shipped cannot be measured into the level. The two are not symmetric, which is the part worth deciding deliberately: B is cheap and one-way, A is expensive and reversible. | open |
| NP-W | **The admission measurement reads this repository's own source, so it can be perturbed by unrelated edits — and it oscillated until it was made a fixed point.** Two failures, both caught by running it twice rather than once. (a) Corpus A originally included `scripts/`, which holds the measurement's own code, so running the measurement changed its own input. `scripts/` and `docs/` are now excluded; the remaining corpus is prose and product source. (b) The candidate pool was `tier === 'contextual'`, so once the generator promoted a detector to `aggressive-only` the next run stopped considering it and the run after demoted it again — the admission list was a function of the last generation instead of a fixed point of (measure → generate). Candidates are now everything outside `standard`, and two full cycles converge to the same list. The remaining exposure — a documentation change shifting the list — is visible in review as a diff on `docs/detector-fp-table.md`, not silent. A frozen corpus snapshot would be the real fix and is not done here. | open |
