...
const SESSION_NODE = { id: 'n', name: 'Privent Session', type: 'n8n-nodes-privent.priventSession' };

function sessionExec(params: Record<string, unknown>) {
  return makeHttpExecFn({
    items: [{ json: {} }],
    params: { framework: 'n8n', webhookNodeName: '', ...params },
    node: SESSION_NODE,
  }).exec;
}
...
describe('PriventSession manual sessionId UUID guard (Option A)', () => {
  it('throws a clear NodeOperationError when manual sessionId is not a UUID', async () => {
    ...
    await expect(new PriventSession().execute.call(exec)).rejects.toThrow(
  ...
  it('accepts a valid UUID in manual mode', async () => {
    ...
    const out = await new PriventSession().execute.call(exec);
